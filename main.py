...
class LangGraphProcessor(FrameProcessor):
    ...
    async def process_frame(self, frame, direction: FrameDirection):
        ...
        #     print(f"[途径帧追踪] -> {frame.__class__.__name__}")

        await super().process_frame(frame, direction)  # 处理通用状态帧

        # 拦截处理LLMContextFrame帧
        if isinstance(frame, LLMContextFrame):
            message = frame.context.messages
            if message and message[-1]["role"] == "user":
                user_last_message = message[-1]["content"].strip()  # 用户最新的话

                if user_last_message:
                    asyncio.create_task(self._run_langgraph(user_last_message))

            return  # 拦截这个帧

        await super().push_frame(frame, direction)  # 其他帧直接往下传
    ...
    async def _run_langgraph(self, user_context: str):
        ...
        try:
            ...
            await self.push_frame(LLMFullResponseStartFrame())  # 发送响应开始帧，通知TTS开始接受数据

            # 流式捕获底层token并实时发送到下游聚合器
            ...
            await self.push_frame(LLMFullResponseEndFrame())  # 发送响应结束帧，合成缓冲区内的剩余帧

        except Exception as e:
            await self.push_frame(LLMFullResponseEndFrame())  # 异常也要正常响应结束
            print(e)
...
async def run_bot(websocket: WebSocket, session_id: str):
    ...
    stt = WhisperSTTService(model = "small", device = "cuda", language = "zh")
    ...
    # 每次连接生成独立的 thread_id
    config = {"configurable": {"thread_id": f"thread_{session_id}", "user_id": "user_1"}}
