...
class DeviceAudioSender:
    ...
    async def play(
        ...
        if self._active is not None:
            await self.abort("superseded by a new playback")
        ...
        try:
            ...
            async for opus_frame in opus_frames:
                ...
                if not self._is_current_and_open(playback):
                    return self._aborted_result(playback)
                await self._websocket.send_bytes(bytes(opus_frame))
            ...
            if not await self._send_stop(playback, require_current=True):
                return self._aborted_result(playback)
            return DevicePlaybackResult(turn_id, generation, "ok", playback.frame_count)
    ...
    async def close(self, *, send_stop: bool) -> None:
        if self._closed:
            return
        self._closed = True
        playback = self._invalidate_active("sender closed")
        if playback is None:
            return
        self._cancel_playback_task(playback)
        if send_stop and playback.started:
            try:
                await self._send_stop(playback, require_current=False)
            except Exception:
                pass
    ...
    async def _send_stop(self, playback: _ActivePlayback, *, require_current: bool) -> bool:
        if playback.stop_sent or not self._transport_is_open():
            return False
